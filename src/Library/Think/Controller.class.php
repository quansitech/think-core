...
abstract class Controller {
   ...
   /**
    ...
    protected function ajaxReturn($data,$type='',$json_option=0) {
        ...
        switch (strtoupper($type)){
            case 'JSON' :
                ...
                qs_exit(json_encode($data,$json_option));
            case 'XML'  :
                // 返回xml格式数据
                header('Content-Type:text/xml; charset=utf-8');
                qs_exit(xml_encode($data));
            case 'JSONP':
                // 返回JSON数据格式到客户端 包含状态信息
                header('Content-Type:application/json; charset=utf-8');
                $handler  =   isset($_GET[C('VAR_JSONP_HANDLER')]) ? $_GET[C('VAR_JSONP_HANDLER')] : C('DEFAULT_JSONP_HANDLER');
                qs_exit($handler.'('.json_encode($data,$json_option).');');
            case 'EVAL' :
                // 返回可执行的js脚本
    ...
    protected function redirect($url,$params=array(),int $delay=0,$msg='',$status=0,$ajax=false) {
        $url    = isUrl($url) ? $url : U($url,$params);
        redirect($url,$delay,$msg,$status,$ajax);
    }
