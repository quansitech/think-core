(function($){
		...
		this.each(function(index, element){
			...
			var uploadManager = {
				...
				init : function(){
					...
					inputStr += '"/>';
					inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="uploadify-button btn btn-primary pure-button button-more button-xsmall">';
				...
				onProgress : function(fileblob, originalfile, loaded, total) {
			  	...
			  	_sendBlob : function(xhr, file, originalfile, url){
			  		if(file.status===0){
                        ...
                        var timeStamp = originalfile.lastModified || originalfile.lastModifiedDate.getTime();
						fd.append('lastModifiedDate', timeStamp);
						if(option.formData){
	...
	const injectFileProp = function (option, uploadedSize, xhr, file, originalFile, uploadManager){
		...
		InjectFileProp.setFileType(option, uploadedSize, xhr, file, uploadManager, need_cacl)
	}
	...
	const InjectFileProp = {
		finish: function f(opt, uploadedSize, xhr, file, upM){
		...
		setHashId: function f(opt, uploadedSize, xhr, file, upM, need_cacl){
			const selfObj = this;
			if (need_cacl){
				window.calc_file_hash(file).then(function(res){
					file.hash_id = res;
					selfObj.finish(opt, uploadedSize, xhr, file, upM)
				});
			}else{
				file.hash_id = '';
				selfObj.finish(opt, uploadedSize, xhr, file, upM)
		...
		setFileType: function f(opt, uploadedSize, xhr, file, upM, need_cacl){
			const selfObj = this;
			if (file.type === ''){
				getFileType.start(file, function f(type){
					...
					selfObj.setHashId(opt, uploadedSize, xhr, file, upM, need_cacl)
				})
			}else{
				selfObj.setHashId(opt, uploadedSize, xhr, file, upM, need_cacl)
			}
